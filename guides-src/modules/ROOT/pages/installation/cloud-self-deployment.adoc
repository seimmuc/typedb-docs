...
mailto:sales@vaticle.com[e-mail,window=_blank] or
https://typedb.com/deploy?dialog=contact[contact form,window=_blank].

// tag::self-hosted-run[]
For deploying with our managed DBaaS offering, see the xref:installation/cloud.adoc[] page.
// end::self-hosted-run[]
...
mailto:sales@vaticle.com[sales team].
...
xref:drivers::overview.adoc[TypeDB Client].

You can use xref:guides::connection/console.adoc[TypeDB Console] from your TypeDB Cloud directory:
...
xref:typedb::managing/ha.adoc#_primary_replica_failover[leader,] and the others are followers.
...
include::guides::partial$kubernetes.adoc[leveloffset=+1]

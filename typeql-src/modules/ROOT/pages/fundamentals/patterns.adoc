...
TypeDB xref:fundamentals/queries.adoc[queries] are written in xref:overview.adoc[TypeQL] and use *patterns* to
...
1. `()` -- parentheses.
See an xref:data/advanced.adoc#_using_parentheses[example].
2. `^` -- exponentiation (power).
See an xref:data/advanced.adoc#_using_exponentiation[example].
3. `*` -- multiplication.
See an xref:data/advanced.adoc#_using_multiplication[example].
4. `/` -- division.
See an xref:data/advanced.adoc#_using_division[example].
5. `%` -- https://en.wikipedia.org/wiki/Modulo[modulo,window=_blank]. Returns the remainder of a division.
See an xref:data/advanced.adoc#_using_modulo[example].
6. `+` -- addition.
See an xref:data/advanced.adoc#_using_addition[example].
7. `-` -- subtraction.
See an xref:data/advanced.adoc#_using_subtraction[example].
...
See xref:data/advanced.adoc#_using_minimum_function[example].
* `max` -- the maximum of the arguments.
See xref:data/advanced.adoc#_using_maximum_function[example].
* `floor` -- the floor function (rounding down).
See xref:data/advanced.adoc#_using_floor_function[example].
* `ceil` -- the ceiling function (rounding up).
See xref:data/advanced.adoc#_using_ceiling_function[example].
* `round` -- the default (half-way up) rounding function.
See xref:data/advanced.adoc#_using_rounding_function[example].
* `abs` -- the modulus (or absolute value) function.
See xref:data/advanced.adoc#_using_absolute_value_function[example].
...
Only one page left to go for fundamentals section:
    ...
    1. xref:fundamentals/inference.adoc[]

If you want to skip inference for now, continue to the *Development* section, starting from the
xref:typedb:ROOT:development/send-queries.adoc[] page.

For more advanced TypeQL patterns, see the xref:data/advanced.adoc[,window=_blank] page in the TypeQL
documentation.

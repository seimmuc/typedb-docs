...
[IMPORTANT]
====
To define a schema, a `schema` xref:typedb:ROOT:development/send-queries.adoc#_sessions[session] must be opened,
and a `write` xref:typedb:ROOT:development/send-queries.adoc#_transactions[transaction] started.
...
[IMPORTANT]
====
The following examples define parts of the xref:typedb:ROOT:tutorials/iam-define-schema.adoc[IAM schema] that will be used throughout
...
xref:typedb:ROOT:home/quickstart-guide.adoc[Quickstart guide] for full descriptions of the following steps:

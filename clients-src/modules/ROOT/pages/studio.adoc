...
//#todo add section on starting Studio
...
// tag::cluster-connect[]
To connect to a TypeDB Cloud or a TypeDB Enterprise cluster,
select TypeDB Cluster option in the btn:[Server] field instead.
Fill the connection parameters and Click the btn:[Connect] button.

image::clients::studio/cluster-connection-interface.png[Connection Manager Cluster Disconnected, width = 70%, link=self]
// end::cluster-connect[]
...
image::clients::studio/studio-database.png[Database icon,width=55%]
...
. Choose the correct xref:typedb:ROOT:development/send-queries.adoc#_sessions[session,window=_blank] (`schema`/`data`) type and
  xref:typedb:ROOT:development/send-queries.adoc#_transactions[transaction,window=_blank] (`read`/`write`) type to run the query
...
. xref:typedb:ROOT:development/send-queries.adoc#_sessions[Session] type switch:
 ** `schema` -- to use schema session type
 ** `data` -- to use data session type
. xref:typedb:ROOT:development/send-queries.adoc#_transactions[Transaction] type switch:
...
. btn:[infer] button -- enable xref:typedb::/development/infer-data.adoc[inferring data] by rules. Only available for read
transactions.
. btn:[explain] button -- enable xref:typedb::development/infer-data.adoc#_explain_query[explanations] mechanics by providing
...
The xref:typeql:ROOT:fundamentals/inference.adoc[inference] transaction option can be switched *on* and *off* with
...
The xref:typedb::development/infer-data.adoc#_explain_query[explanation] transaction option can be switched
...
All types are divided in three top level xref:typeql:ROOT:fundamentals/types.adoc#_type[built-in types]:
...
For example, we can't xref:typedb::development/define-schema.adoc#_undefine_subtype[delete a type that has a subtype].
====

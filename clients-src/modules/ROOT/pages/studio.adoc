...
. Choose the correct xref:typedb:ROOT:fundamentals.adoc#_sessions[session,window=_blank] (`schema`/`data`) type and
  xref:typedb:ROOT:fundamentals.adoc#_transactions[transaction,window=_blank] (`read`/`write`) type to run the query
...
. xref:typedb:ROOT:fundamentals.adoc#_sessions[Session] type switch:
 ** `schema` -- to use schema session type
 ** `data` -- to use data session type
. xref:typedb:ROOT:fundamentals.adoc#_transactions[Transaction] type switch:

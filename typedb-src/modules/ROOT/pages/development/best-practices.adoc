...
* Limit the number of concepts being processed by adding additional
  xref:typeql:ROOT:fundamentals/patterns.adoc#_patterns_overview[constraints] to variables in `match` clauses.
* Limit the number of results returned by using pagination (limit + offset) or
  xref:typeql:ROOT:fundamentals/queries.adoc#_modifiers[aggregation] when possible.
...
xref:bin/response.adoc#_conceptmap[ConceptMaps]. Use the methods introduced by the TypeDB Driver API to obtain more
...
Using an optional xref:typedb::development/read-data.adoc#_get[get] clause can alter the set of returned results. For example:
...
We can further improve the output by xref:typeql:ROOT:fundamentals/queries.adoc#_modifiers[grouping] the results by `person`
and/or applying xref:typeql:ROOT:fundamentals/queries.adoc#_modifiers[aggregation] to count the number of results.

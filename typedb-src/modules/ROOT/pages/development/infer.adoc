...
Inference is enabled as a Read xref:fundamentals.adoc#_transactions[transaction] option.
...
By default, the IAM dataset used in the xref:quickstart/studio.adoc[Quickstart guide] sets permissions only for the
`modify_file` action.
...
xref:quickstart/studio.adoc[Quickstart guide]) can infer `view_file` permission for multiple files for a single
...
the result of 10 files (by default in the IAM database from the xref:quickstart/studio.adoc[Quickstart guide]).

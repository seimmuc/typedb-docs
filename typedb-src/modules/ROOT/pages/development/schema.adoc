...
2. xref:fundamentals.adoc#_clients[Connect] to a TypeDB database with a xref:fundamentals.adoc#_sessions[SCHEMA session].
3. Run a proper query type (xref:fundamentals/queries.adoc#_define_query[Define]/ xref:fundamentals/queries.adoc#_define_query[Undefine])
   with a query string we built in a xref:fundamentals.adoc#_transactions[WRITE transaction].
...
xref:quickstart/studio.adoc[Quickstart] page), or we can define them in the same query. To do that, use the following query
instead:

...
Inference is enabled as a Read xref:development/send-queries.adoc#_transactions[transaction] option.
...
By default, the IAM dataset used in the xref:home/quickstart-guide.adoc[Quickstart guide] sets permissions only for the
`modify_file` action.
...
xref:home/quickstart-guide.adoc[Quickstart guide]) can infer `view_file` permission for multiple files for a single
...
the result of 10 files (by default in the IAM database from the xref:home/quickstart-guide.adoc[Quickstart guide]).

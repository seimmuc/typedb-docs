...
TypeDB instances can contain multiple databases. A database consists of a xref:development/define-schema.adoc[schema] and
xref:development/write-data.adoc[data]. It is both separate and independent of any other database: two databases can't influence
...
For more information on these limits, see the xref:development/acid-guarantees.adoc#_schema_integrity[] section.
====
...
xref:development/acid-guarantees.adoc[] up to xref:development/acid-guarantees.adoc#_isolation[snapshot isolation].
...
read/write transactions. For more information, see the xref:development/acid-guarantees.adoc[] page.
...
After that we can try xref:development/write-data.adoc[writing] and xref:development/read-data.adoc[reading] data from
a database.

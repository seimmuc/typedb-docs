...
[#_inference]
TypeDB is capable of reasoning over data via rules xref:development/define-schema.adoc#_define_rules[defined] in the schema.
...
When executing a `get` query with the `infer` transaction option xref:development/infer.adoc#_how_to_use_inference[enabled]
...
Read more on how to create rules in a schema in the xref:typedb::development/define-schema.adoc#_define_rules[Define rules]
section of the xref:typedb::development/define-schema.adoc[] page.

We can use xref:typedb::fundamentals/patterns.adoc#_computation[computation] operations and functions in the condition pattern.
And we can use value variables in the conclusion of a rule.

[#_rule_validation]
=== Rule validation

include::typeql::schema/define-rules.adoc[tag=rule_validation]

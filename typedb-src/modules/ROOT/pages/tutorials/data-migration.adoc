...
developed a xref:development/define-schema.adoc[schema] for the TypeDB database according to the data model. Accurate and
...
To load data into a TypeDB database we need to perform xref:typedb::development/write-data.adoc#_insert[insert] queries.
...
xref:home/quickstart-guide.adoc[Quickstart guide]. Make sure to use the schema file provided above instead of the IAM schema

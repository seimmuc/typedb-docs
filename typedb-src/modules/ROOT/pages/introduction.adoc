...
xref:fundamentals.adoc#_acid_guarantees[ACID] guarantees.
* Has data separated by xref:fundamentals.adoc#_databases[databases], each one with its own schema.
...
1. First, use the xref:install-and-run/install.adoc[installation guide] to install TypeDB on the local machine.

2. The TypeDB journey starts with our xref:quickstart/studio.adoc[Quickstart guide].
+
...
4. xref:fundamentals.adoc[Connect] page: Connect to TypeDB using databases, sessions, transactions, and ACID
   guarantees.
